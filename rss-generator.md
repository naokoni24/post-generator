...
- 記事URLは「投稿文（編集可）」欄の下に別枠（🔗 リプライ用URL）として表示する
- 「X で投稿」ボタンを押すと、URLなしの本文でX投稿画面を開くと同時に、記事URLをクリップボードへ自動コピーする（intentベースの投稿のため、投稿後のツイートIDをアプリ側で取得できず、リプライの自動投稿はできない。ユーザーは投稿後、自分のツイートにリプライしてクリップボードのURLを貼り付ける運用）
- 「📋 リプライ用URLをコピー」ボタンでも同じURLを手動コピーできる
- 投稿履歴（今日の投稿履歴）にはリプライ用URLを保存しないため、履歴から復元した場合はURL欄は非表示になる
...
- 【フォロワー増加のためのUX変更・URLを本体ツイートからリプライへ】「毎日投稿しているのにXのフォロワーが増えない」という相談を受け、外部リンクを含む投稿はXのアルゴリズム上リーチが下がりやすいという通説に基づき、URLの扱いを変更した
  ...
  - 結果カードに新しく「🔗 リプライ用URL」枠（`replyUrlRow`/`replyUrlValue`/`copyUrlBtn`）を追加。記事URLをここに表示し、「📋 リプライ用URLをコピー」ボタンで手動コピーできる
  - 「X で投稿」ボタン（`xBtn`）押下時に、URLなし本文でXの投稿画面を開くのと同時に記事URLを自動でクリップボードにコピーする（Twitter intent URLベースの投稿のため、投稿後のツイートIDをアプリ側で取得できず、自己リプライの自動投稿はできない。運用はユーザーが本体を投稿→自分のツイートにリプライしてクリップボードのURLを貼り付ける、の2ステップ手動）
  - 投稿履歴（`postHistory`/`markPosted`）にはリプライ用URLを保存していないため、履歴から復元（`loadHistory`）した場合は`replyUrlRow`を非表示にする
  - 検証: `node --check`でJS構文確認、Python構文チェック、サーバー実起動でHTML配信確認。GEMINI_API_KEY未設定のためGemini実呼び出し込みの完全なE2Eは未検証だが、ブラウザでDOM操作により結果カード表示後の状態を再現し、リプライURL枠の表示・本体ツイート本文にURLが含まれないこと・「📋 リプライ用URLをコピー」ボタンの実クリック（実際のユーザー操作扱いでのクリップボード書き込み）が正常動作することを確認済み
