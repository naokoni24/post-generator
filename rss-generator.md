...
- 取得先選択
  - `国内`
  - `海外`
- `公式優先` チェックボックス（チェック時、候補一覧で公式Blog / GitHub Releases / Docs更新を上位表示）
- 期間選択
  - 今日
  - 1日以内
  - 3日以内
  - 1週間以内
- 感想スタイル（一言感想 / 問いかけ / 実務目線 / 懸念・考察。デフォルトは実務目線）
...
記事の選択は単一選択のみです（候補カードをクリックすると以前の選択は解除され、常に1件だけが選択されます）。
...
同じ検索条件（カテゴリ・国内/海外・期間・キーワード）で今回の取得結果が20件未満の場合は、当日中に同条件で取得済みの候補をブラウザの`localStorage`から重複なしで補完し、最大20件表示します。今回取得した候補を常に優先し、日付が変わった場合や検索条件が異なる場合は候補を混ぜません。過去候補自体が20件に満たない場合は、最終表示も20件未満になることがあります。
...
「公式Xも見る」チェックボックスはUIから削除済みで、現在 `include_x` は常にオフです（公式X候補は通常表示されません）。サーバー側の生成ロジック（`get_official_x_candidates`）とアカウント設定は将来の再有効化に備えて残しています。

公式XはX検索URLを候補として出す設計です。アプリ側でXの投稿本文を直接スクレイピングする構成ではありません。
...
- 今回の有効候補が20件未満の場合は、当日中の同一検索条件の過去候補で最大20件まで補完する
...
感想スタイルの下に「投稿の長さ」選択（`POST_LENGTHS`）があり、長文・短文のどちらで生成するか選べます。フォロワーの少ない段階では長文がタイムラインで「さらに表示」に折りたたまれて最後まで読まれにくいという見方があるため、長文一択をやめて短文モードとの比較（後述の効果測定ループ）ができるようにしています。デフォルトは長文。

- 📝 長文（デフォルト）: 日本語350〜500文字程度、3〜5文。短すぎる場合（重み600未満、日本語約300文字未満）はGemini APIで本文だけを一度補強してから表示する
- ⚡ 短文: 日本語120〜180文字程度。短すぎる場合の自動補強（膨張）は行わない（短く作るのが目的のため）
- 文字数を埋めるための水増しはせず、具体的な情報で自然に厚みを持たせる（両モード共通）
- ハッシュタグは自動付与しない（両モード共通）

### URLの扱い（本体ツイートには含めない）

外部リンクを含む投稿はXのアルゴリズム上リーチが下がりやすいと言われているため、本体ツイート（`tweetBox`・「X で投稿」ボタンで開く内容）には記事URLを含めません。

- 記事URLは「投稿文（編集可）」欄の下に別枠（🔗 リプライ用）として表示する。中身は生のURLだけではなく`buildReplyText()`が組み立てる「元記事はこちら👇\n{URL}」という2行のリプライ文
- 「X で投稿」ボタンを押すと、URLなしの本文でX投稿画面を開くと同時に、上記のリプライ文（「元記事はこちら👇」＋URL）をクリップボードへ自動コピーする（intentベースの投稿のため、投稿後のツイートIDをアプリ側で取得できず、リプライの自動投稿はできない。ユーザーは投稿後、自分のツイートにリプライしてクリップボードの内容を貼り付ける運用）
- 「📋 リプライ文をコピー」ボタンでも同じ内容を手動コピーできる
- 投稿履歴（今日の投稿履歴）にはリプライ文を保存しないため、履歴から復元した場合はリプライ欄は非表示になる

### リプライ先候補（フォロワー増加のための導線）

フォロワー増加は自分の投稿だけでなく他人のリプライ欄での露出が大きく効くと言われているため、投稿文生成後に結果カードへ「💬 リプライ回り候補」を表示します（`renderReplyTargets()`）。API呼び出しは発生せず、リンクを組み立てるだけです。

- 選択中のカテゴリ（`activeCat`）に対応する公式Xアカウント（サーバー側`OFFICIAL_X_ACCOUNTS`と同じ内容をJS側`OFFICIAL_X_ACCOUNTS`に複製）へのプロフィールリンクを2〜3件表示
- 記事タイトル（先頭60文字）でのX検索リンク（`https://x.com/search?q=...&f=live`、最新順）も1件表示し、同じ話題の投稿を見つけてリプライできるようにする
- カテゴリ未選択（キーワード横断検索など）で該当アカウントがない場合や記事タイトルが空の場合は、それぞれ表示をスキップする
- 投稿履歴から復元した場合はリプライ用URLと同様に非表示にする（履歴には保存していないため）

### 画像生成プロンプト（3種類のテンプレートから自動選択）

結果カードの「🎨 画像生成プロンプトを作成」ボタン（`imgPromptBtn`）は、実際の画像は生成せず、外部の画像生成AIに渡すための英語プロンプト文字列を1回のGemini呼び出しで組み立てます。

以前は記事内容に関わらず常に「マスコットロボット＋矢印でつないだセクション」という1種類の構図固定だったため、毎日投稿すると画像が代わり映えせず見飽きられやすいという課題があった。これを解消するため、記事の性質に応じて3種類のテンプレートから選ぶようにした。

- Geminiへの1回の呼び出しで、記事内容から`template`（`comparison`/`stat`/`announcement`のいずれか）を判定させつつ、選んだテンプレートに必要なフィールドだけを埋めたJSONを取得する（API呼び出し回数は従来と変わらず1回のまま）
  - `comparison`（比較・変化型）: Before/After・旧方式→新方式・課題→解決策など、明確な変化の流れがある記事向け。従来通りの「マスコットロボット＋横並びセクション＋矢印＋キラキラ装飾」の構図（`buildComparisonImagePrompt()`）
  - `stat`（数値インパクト型）: 「〇〇%削減」「〇倍高速化」のような明確なインパクト数値が記事の中心である場合向け。巨大な数字を主役にした、装飾を抑えたシンプルなstat-cardデザイン（`buildStatImagePrompt()`）
  - `announcement`（発表・アップデート型）: 上記どちらにも当てはまらない、比較や強調数値のないシンプルな発表・お知らせ記事向け。マスコットやキラキラを使わない、落ち着いたエディトリアル風のカードデザイン（`buildAnnouncementImagePrompt()`）
- `template=stat`なのに`stat_value`/`stat_label_ja`が欠けている、`template=announcement`なのに`body_ja`が欠けているなど、必要フィールドが揃わない場合は`comparison`テンプレートにフォールバックする（`buildImagePromptFromParsed()`）
- 3テンプレートとも、共通の技術仕様（16:9・1200x675・Xの1枚画像添付向け安全マージン）と、末尾の「AIで生成したとわからないようにして」の指示文（`IMG_BASE_SPEC`/`IMG_FOOTER`）を共有する

### 文字数オーバー時の処理

1. 本文だけで4,000文字相当（`POST_CHAR_LIMIT`）を超える場合は Gemini API で本文だけを自動短縮
...
- 日本語・CJK文字: 2文字扱い
- 基本ラテン文字など: 1文字扱い
- URL: 23文字扱い（本体ツイートには通常URLを含めないため、基本的にこの加算は発生しない。手動でURLを本文に貼り付けた場合のみ適用される）

上限（4,000文字相当）を超えた場合は自動短縮ボタン（✂️ 自動短縮）が表示されます。

## 投稿の効果測定ループ

「今日の投稿履歴」とは別に、どの感想スタイル・カテゴリ・長さの投稿が伸びているかを日をまたいで振り返れるよう、投稿のたびに記録を1件作成し、あとから表示回数・いいね数を手入力できる「📈 投稿の効果を記録」セクションを画面下部に設けています。

- 保存キー: `it_post_metrics_log_v1`（localStorage、直近200件まで、日付が変わっても消えない）
- 「X で投稿」ボタンを押した時点で記録を1件作成（`addMetricsEntry()`）。内容: 記事タイトル・カテゴリ・感想スタイル（感想を含めない場合は空）・投稿の長さモード・日時。表示回数／いいね数は`null`（未入力）で初期化
- 記録一覧（直近15件）に表示回数・いいね数の入力欄があり、`onchange`で確定するたびに保存・再集計する（入力途中の再描画によるフォーカス喪失を避けるため`oninput`ではなく`onchange`を使用）
- 集計は「感想スタイル別」「カテゴリ別」「投稿の長さ別」の3軸で、それぞれ平均表示回数・平均いいね数・件数を表示（`aggregateMetrics()`）。表示回数・いいね数が両方とも未入力の記録は集計対象から除外する
- 「今日の投稿履歴」（`postHistory`）とは独立したデータのため、こちらを削除しても投稿履歴・投稿文の再利用（`loadHistory`）には影響しない
...
| `POST` | `/api/claude` | Gemini API経由で投稿文などを生成（パス名はClaude利用時代の名残） |
...
- RSS取得キャッシュ: 5分
- 取得失敗フィードの一時スキップ: 10分
- 記事本文キャッシュ: 30分（`(URL, 文字数上限)`単位。本文確認と投稿文生成で再利用）
- 翻訳キャッシュ: `translation_cache.json` にディスク永続化（TTLなし・最大3,000件・バージョンキー付き。`title_ja`が空のエントリは読み込み時に除外）

結果キャッシュ（同一検索条件の30分キャッシュ `_RESULT_CACHE`）は候補選定の簡素化リファクタで廃止されました。
...
- RSSフィード取得: フィード数に応じて最大30以上の並列（全タスクを同時実行）
- 本文確認: 最大20並列
- 翻訳: バッチサイズ20件・直列実行（Gemini Flash-Liteのレート制限429を避けるため）
...
- 有効期間: 最終操作から `SESSION_IDLE_TIMEOUT_SECONDS` 秒（デフォルト30分）。認証済みアクセスごとにCookieを再発行して延長するアイドルタイムアウト方式
- 署名方式: HMAC SHA-256（最終操作時刻を署名付きトークンとして保持）
...
- 【候補20件未満時の過去結果補完】RSS取得のタイムアウトや本文確認により今回の候補が20件未満になった場合、当日中に同じカテゴリ・国内/海外・期間・キーワードで取得した候補を`localStorage`から補完し、URL（URLがない場合はソース＋原題）で重複除外して最大20件表示するよう変更。新規取得分を先頭に保ち、検索条件の異なる候補や前日以前の候補は混ぜない
...
- 【本文取得の精度改善】投稿文生成時に記事本文をより正確・広範囲に読み取るよう `fetch_article_body()` を改修
  - `<article>`タグが存在するページでは、そのブロックのうち最も長いものだけを本文として採用するように変更。従来はページ全体からタグを剥がしていたため、ナビ・広告・関連記事プレビューなどのノイズが文字数上限を消費し、肝心の本文が削られやすかった（`<article>`が無い/短すぎる場合はページ全体抽出にフォールバック）
  - 本文取得の文字数上限を3,000文字→6,000文字に拡大し、長い記事でも後半の情報（数値・結論など）まで読み込めるようにした
  - `/api/fetch_article` エンドポイントも新しいデフォルト値（6,000文字）を使うよう修正（従来は3,000文字を明示指定していた）
  - 【リモート追随時の整合性修正】その後リモート側で `fetch_article_body()` に `(url, char_limit)` 単位の本文キャッシュ（`_ARTICLE_BODY_CACHE`）とURL事前検証（`filter_candidates_with_article_body`、`char_limit=3000`固定）が追加されていたため、事前検証側の `char_limit` も6,000に揃えた。これを揃えないと事前検証時とその後の投稿文生成時でキャッシュキーが一致せず、同じURLを二重に取得してしまう
  - 【実サイトでの検証・追加修正】ITmedia・TechCrunch・Zenn・Ars Technicaの実記事URLで抽出結果を検証したところ、ITmedia記事の多くは`<article>`タグを使っておらずページ全体抽出のままだった一方、TechCrunchは`<article>`が無く`entry-content`クラスのdivに本文が入っており、文字数上限を広げただけでは「関連記事へのリンク」「読み込みエラー」等のノイズも一緒に多く取り込んでしまうことを実測で確認した。対策として、`<article>`タグが無い場合の第2候補として`entry-content` / `article-body` / `post-content`等の一般的なクラス名を持つブロックも検出するように追加。ネストしたdivの正確な終了位置を深さ計算で求める方式は、実サイトのHTMLは閉じタグの対応が崩れていることが多く機能しなかったため不採用とし、開始位置から固定文字数のHTMLだけを対象にする簡易的な方式を採用。windowの終端がタグの途中で切れると閉じられていないタグの断片が本文に混入するバグも発見し、最後の完全なタグ境界で切り直す処理を追加して修正した
- 【本文抽出の大規模調査・第3世代ロジック】アプリで実際に使用している主要RSSソース31サイトから実記事を1件ずつ取得し、本文がどのHTML構造に入っているかを網羅調査。結果に基づき `_extract_main_content_block()` を「①`<article>`タグ最長ブロック → ②本文系クラス/IDパターン → ③`<main>`タグ → ④ページ全体」の4段階カスケードに再設計した
  - 調査で判明したサイト実態: The Hacker Newsは属性が**シングルクォート**（`class='articlebody'`）で従来パターン（ダブルクォート限定）が効かない。Publickeyは`entrybody`（ハイフン無し）、CodeZineは`c-article_content`（アンダースコア）、The New Stackは`tns-post-body`。ITmedia/Kubernetes Blog/GitHub Blogはどのクラスパターンにも合わないが`<main>`タグがある。Stack Overflow BlogはJSレンダリングでサーバーサイド取得自体が不可（対応不能）
  - クラスパターンを `entry[-_]?content|entry[-_]?body|article[-_]?body|article[-_]?content|post[-_]?content|post[-_]?body|main[-_]?content`（クォート両対応・大文字小文字無視）に拡張。ハイフン/アンダースコア/連結の揺れをすべて吸収
  - 同名クラスが複数箇所にあるサイト（CodeZineの導入ボックス+本文など）や最初のマッチが隠しテンプレートのサイトがあるため、パターンは先頭8マッチまで試して**テキスト量最長のwindow**を採用
  - windowサイズは20k→30kに拡大（The Hacker News等マークアップが重いサイトで本文後半が切れるため。40kは関連記事ノイズが増えるので不採用）
...
- 【カテゴリ検索の件数が少ない事象を調査】全8カテゴリ×国内/海外で候補件数をテストし、原因を切り分け
  - **修正した実装上の無駄**: `/api/rss`ハンドラが1リクエスト内で最大5回`_load_articles()`を呼び直す設計だが、再試行のたびに`_RSS_FAIL_CACHE.clear()`していたため、HTTP 403（例: BleepingComputer、TechRepublic）や接続切断（例: How-To Geek、MakeUseOf）など**確実に失敗するフィード**を毎回律儀に再試行し、後続の取得予算を浪費していた。初回の`clear()`のみ残し、リクエスト内の再試行では失敗キャッシュを保持するよう修正。Previewで同一フィードへの重複失敗ログが減り、応答も高速化したことを確認
  - **コードの問題ではなく仕様・コンテンツ量に起因するもの**（今回は変更せず）:
    - セキュリティ/国内・スタートアップ/国内・ビジネスDX/国内は、そのカテゴリの国内ソースが2〜3件しかなく、かつ「今日」当日に該当ソースが更新されないと件数が少なくなる。「今日」指定では前日以前への自動補完をしない仕様のため（意図的）
    - ガジェット・ハードウェア/海外は、有効な海外ソースが少ない（Engadget、The Verge等）ことに加えGoogle News Hardware等がタイムアウトしやすい
    - 恒常的に403/接続切断するフィード（BleepingComputer、TechRepublic、How-To Geek、MakeUseOf）は将来的に別途、代替フィードへの差し替えや削除を検討する余地がある
- 【国内ソースを追加】前述の調査で判明した「国内ソースが2〜3件しかない」カテゴリ（セキュリティ、スタートアップ、クラウド・AWS、ガジェット・ハードウェア）にソースを追加
  - 追加前に`curl`で全URLの疎通・記事件数を確認してから採用（ZDNet Japan系のRSSは404で廃止済みと判明し見送り）
  - セキュリティ: Security NEXT、JPCERT/CC注意喚起、Google News セキュリティ（日本語検索）を追加
  - スタートアップ: Coral Capital（VCブログ）、Google News スタートアップ（日本語検索）を追加
  - クラウド・AWS: Google News クラウド（日本語検索）を追加
  - ガジェット・ハードウェア: AV Watch、Google News ガジェット（日本語検索）を追加
  - 便利ツール・Tips: 窓の杜を追加
...
- 【カテゴリ取得の早期終了バグ修正】海外カテゴリ検索で件数が異常に少なくなる事象（例: クラウド・AWSが1件のみ）を`get_articles()`で調査・修正
  - 原因: RSS取得の早期終了判定`_recent_candidate_count()`が、取得済み**全件**（言語フィルタ・カテゴリ関連度フィルタ適用前）で件数を数えていた。海外(`en`)検索では応答の速い国内フィード（はてブ IT、Publickey等）が先に完了し、その時点で（後で言語フィルタにより除外される）件数だけで`limit`に達してしまい、まだ完了していない海外フィードのfutureを全てキャンセルしていた。結果、フィルタ後に残る海外候補が数件しかなくなっていた
  - 修正: `_recent_candidate_count()`を「最終的に候補として残りうる記事」（`lang`による言語フィルタ・カテゴリ関連度フィルタを適用した後）だけで数えるように変更。実機検証で海外(en)・今日指定の全8カテゴリのうち、クラウド・AWS 1→20件、ガジェット・ハードウェア 0→17件、ビジネス・DX 0→20件など大幅に改善し、国内ソースの混入も無いことを確認
- 【候補選択ズレバグ修正】AWSカテゴリ等で、クリックした記事と異なる記事が選択されてしまう不具合を修正
  - 原因: `renderCands()`で「公式優先」チェックボックスがオンの場合、表示用に`displayCandidates`を並べ替えていたが、各カードの`onclick="selectCand(i)"`にはその**並べ替え後の表示位置**を渡していた。一方`selectCand`や本文取得・投稿文生成処理は渡された値を元の`candidates`配列への添字としてそのまま使うため、並べ替えで表示順と元の配列順がずれると、クリックした記事とは別の記事（元の配列で同じ位置にあった記事）が選択・生成されていた。公式ソース（AWS Blog、GitHub Releasesなど）が多いAWS/クラウドカテゴリは「公式優先」使用時に並べ替えが起きやすく顕在化しやすかった
  - 修正: 並べ替え前に元のインデックス（`origIdx`）を各候補に保持させ、`onclick`・選択状態判定（`selectedIndices.indexOf`）には常に`origIdx`を、カード左上の順位表示（`cand-num`）には表示位置（`pos+1`）を使うよう分離
  - 検証: Node.jsで新旧ロジックを実データで再現。旧コードでは「公式優先」ON時にクリックした記事と異なる記事が選択されることを再現し、修正後は一致することを確認。「公式優先」OFF時（従来の通常時）や複数記事選択（2〜3件のオリジナル記事作成モード）のトグルON/OFFも含めリグレッションが無いことを確認済み
- 【画像生成プロンプトの精度改善】本文抽出の精度向上（`<article>`優先抽出・6,000文字化）は`fetch_article_body()`経由で画像プロンプト生成にも間接的に効いていたが、画像プロンプト側は`source.slice(0,2500)`で先頭2,500文字に切り詰めたままだったため、6,000文字化の恩恵を活かしきれていなかった
  - 投稿文生成用のプロンプト（`contextText`）は元々スライスせず`articleBody`全体を渡している一方、画像プロンプト側だけ独自に2,500文字上限を設けていた不整合を発見
  - `.slice(0,2500)`を削除し、投稿文生成と同じく`fetch_article_body()`側の上限（6,000文字）にそのまま従うよう統一。先頭2,500文字だけでは記事後半の「変化・結論」を見落とし、テンプレ的なBefore/After構成になりがちだった問題を解消
  - 変更はHTML内埋め込みJSの文字列テンプレート差し替えのみのため、`node --check`でJS構文が壊れていないことを確認済み（Gemini呼び出し自体はAPIキー未設定のためローカルでは未検証）
- 【重大バグ修正・翻訳キャッシュ永続汚染】特定の見出しが日本語表示に失敗し続ける不具合を調査・修正
  - 原因: `translate_titles()`内の`_apply_translations()`が、Geminiのバッチ翻訳レスポンスで`title_ja`が空文字だった項目も無条件で「翻訳成功（applied）」扱いにしていた。その結果、(1)本来ある漏れ再試行（`missing_targets`）の対象から除外されて即座に見捨てられ、(2)空文字のまま`(タイトル,概要)`をキーに翻訳キャッシュ（`translation_cache.json`、TTL無しでディスク永続化）へ書き込まれてしまっていた。一度この状態になった見出しは、以後同一の(タイトル,概要)を持つ記事が来るたびキャッシュヒットして英語表示のまま二度と再翻訳されなくなる（サーバー再起動しても`translation_cache.json`が残る限り直らない）
  - 修正: `_apply_translations()`で`title_ja`が空の項目は何もせずスキップするよう変更（キャッシュにも書き込まない）。これにより漏れ再試行の対象に残り、再試行でも失敗した場合はその回だけ英語表示のまま（キャッシュ汚染はしない）優雅に縮退し、別の検索・別プロセスでは改めて翻訳を試みられる
  - 合わせて`_load_translation_cache()`に`title_ja`が空のエントリを読み込み時に除外するガードを追加。修正前にすでに`translation_cache.json`へ書き込まれてしまった汚染エントリがあっても、次回サーバー再起動時に自動的に無視され自己修復する
  - 検証: モジュールをコピーして`_translate_batch`をモック化し、(1)空文字レスポンスがキャッシュに永続保存されないこと、(2)同一見出しを別プロセス相当で再度渡すと今度は正しく翻訳が試みられ成功すること、(3)修正前バージョンのファイルで同じテストを実行するとバグが再現する（`('', '')`が永続キャッシュされ、2回目の呼び出しがキャッシュに阻まれてGeminiに問い合わせすらされない）ことを確認。また、あらかじめ汚染エントリを仕込んだ`translation_cache.json`を用意し、読み込み時に除外されること・その記事が次回アクセスで正しく再翻訳されることも別途確認済み
- 【公式記事が候補に残りにくいバグ修正】「AIカテゴリで検索すると公式記事が少ない」という報告を受け、候補選定ロジックを調査
  - 原因1: 直近の「候補選定を全件取得→新しい順にlimit件のシンプルな方式に変更」というリファクタで、最終選定に使う`sort_articles_newest_first()`が単純に`(sortTime, trustScore)`のみで並べる実装になっていた。信頼度（trustScore）はsortTimeが完全に一致した場合のタイブレークにしか使われず実質機能しないため、更新頻度の低い公式Blog（AI企業各社は1日1本程度）は、更新頻度の高いRSSニュース集約ソース（数十フィード）に新しさで負け続け、母集団に存在していても最終20件からほぼ排除されていた
  - 原因2: 重複統合前の`unique.sort(key=_article_sort_key)`（同一ニュース統合時にどの記事を代表として残すかを決める並び）でも、`official_blog`だけが優先タイの対象から漏れていた（`github_release`/`docs_update`/`official_x`のみ優先扱いで、`official_blog`は通常のRSSニュースと同列だった）。AI・機械学習カテゴリは`DOCS_UPDATE_FEEDS`が空、`GITHUB_RELEASE_FEEDS`も2リポジトリのみで、公式系のほとんどが`official_blog`（OpenAI Blog、Anthropic Blog、Google DeepMind Blog等）に集中しているため、この抜けの影響を最も受けやすいカテゴリだった
  - 修正: `_article_sort_key`の優先タイ対象に`official_blog`を追加。`sort_articles_newest_first()`を、公式系（`official_blog`/`github_release`/`docs_update`/`official_x`）を優先タイにしてから、タイ内は引き続き新しい順にするよう変更（公式系が完全に独占しないよう、あくまで「同じタイ内は新しい順」の緩やかな優先度にとどめ、以前のカテゴリ・ソース種別ごとの厳密な上限（type_caps）のような複雑な仕組みは再導入しない）
  - 検証: 実データで複数カテゴリを比較。AI・機械学習(海外)は公式系1→3件/20件に増加。開発(海外)6件/19件、クラウド・AWS(海外)8件/18件など、公式系が一定数含まれつつ独占はしないバランスになっていることを確認。AI・機械学習(国内)は公式系が0件のままだが、これは公式ブログが軒並み海外ソースのため「国内/海外完全分離」仕様により正しく除外されているだけで正常（該当カテゴリに国内公式ソースが存在しないため）
- 【複数記事統合機能を削除】候補選択と投稿生成を、1記事から1投稿を作る単一選択フローへ戻した
  - 候補カードを選ぶと以前の選択が解除され、常に1件だけが選択状態になる
  - 複数記事統合用の状態管理・生成プロンプト・複数URL付加・代表記事オブジェクトを削除
  - 投稿文生成、画像生成プロンプト、投稿履歴は選択した1記事のタイトル・本文・URLだけを使用する
- 【全体健全性調査・クリーンアップ】「他に変なところがないか」の依頼を受け、コードと仕様書を体系的に点検
  - コード検査: JSの`el()`参照とHTMLのid、onclickで呼ばれる関数の存在をすべて突き合わせ、不整合ゼロを確認。サーバーを実際に起動して全エンドポイント（`/` `/login` `/api/status` `/api/rss` `/api/cancel` `/api/fetch_article` `/api/claude` `/api/translate_candidates`、アイコン・manifest配信）の応答とエラーパスを確認、異常なし
  - デッドコード削除: ハッシュタグ機能廃止時に残っていたJSの`getTags()`と`tags`変数、旧実装のPython関数`fetch_feed_group()`（`get_articles`内の`_fetch_group`に置き換え済み）と`is_english()`（`needs_translation`に置き換え済み）を削除
  - 軽微修正: 候補翻訳後の置き換えで選択記事が見つからなかった場合に、古いタイトルのスティッキーバーが残り得たため`updateStickyBar()`を呼ぶよう修正
  - 仕様書本文の乖離を修正: リモートセッションでの大規模リファクタ時に変更履歴だけ追記され本文が古いままだった箇所を現状に合わせて更新（入力項目の「公式Xも見る」削除と「公式優先」追記・感想スタイルの重複記載解消・単一選択の明記、type_caps表→公式系優先タイの説明、並び順セクションに公式系優先タイと本文確認3倍プールを反映、タイムアウト値1.8s/1.2s/2.6s→4.0s/4.0s/8.0s等の現行値、翻訳バッチ5件6並列→20件直列、結果キャッシュ30分の廃止と本文キャッシュ・翻訳永続キャッシュの追記、認証Cookie有効期間7日→アイドルタイムアウト30分方式、投稿文生成/短縮のClaude API表記→Gemini API、`/api/claude`はパス名がClaude時代の名残である旨を注記）
  - 軽微な既知事項（修正せず記録のみ）: 存在しないパスへのGETは404ではなくメイン画面を200で返す。`/api/rss`の`days`はUI外の値（999等）も検証なしで受け付ける（UIからは0/1/3/7しか送られないため実害なし）。不正なカテゴリ名はAI・機械学習にフォールバックする
- 【ブラッシュアップ6件】提案した改善案をまとめて実装
  - 切り口・構成の生成を2回のAPI呼び出しから1回に統合: 従来は③独自の切り口決定→④記事構成作成を逐次2回呼び出していたが、④は③の結果を使うだけで対話的な調整は不要なため、`{"angle": "...", "outline": [...]}`形式のJSONを1回の呼び出しでまとめて取得するよう変更。API呼び出し回数とレイテンシを削減（本文生成と合わせて計3回→2回）
  - 【調査の上で見送り】Google News経由の記事（本文取得不可でRSS概要のみになる）について、URLのbase64部分をデコードして実記事URLを取り出す案を検証したが、Googleが数年前にURL形式をprotobufベースに変更しており単純デコードでは復元できないことを確認。リダイレクト先ページにも実URLは埋め込まれておらず、非公開のbatchexecute API呼び出しが必要な形式のため、フラジャイルな非公式APIへの依存を避けて見送った
  - 【重大・不具合修正】Anthropic Blogフィードが「Log in | Verification Portal - portal.anthropic.com」等のログインページを記事として拾っていた問題を修正。原因は検索クエリが`site:anthropic.com`とパス指定なしだったため。`site:anthropic.com/news`にパスを限定して解消（実データで確認: ログイン/ポータルページの混入ゼロ、実記事のみ取得）。あわせて更新頻度の低さで`when:3d`だと0件になりやすかったため`when:14d`に拡大（実際に何日以内の記事として表示するかは引き続きアプリ側の`recent_days`判定に従うため、鮮度の扱いは変わらない）
  - 投稿履歴（今日の投稿履歴）を`localStorage`に永続化: 従来はブラウザのJS変数のみでリロード・PWA再起動で消えていた。`it_post_history_v1`キーで`{date, items}`を保存し、読み込み時にJST日付が今日と一致する場合のみ復元する（ラベル通り「今日の」履歴として扱うため、日付が変わっていたら破棄）。壊れたJSON・localStorage利用不可環境でも例外にならずグレースフルに空扱いにする
...
- 【関連プロジェクト作業メモ】「うちの子ノート」にローカル限定デモモードを追加
...
- 【画像プロンプト調整】画像生成プロンプト（`imgPromptBtn`クリック時に組み立てる`finalPrompt`）の最終行に「AIで生成したとわからないようにして」という一文を追加。生成した画像プロンプトをそのまま画像生成AIに渡した際、AI生成であることが分かりにくい見た目に寄せる指示を明示的に含めるようにした
  - 追記時は本文（構図・スタイル指定）と区切るため、間に空行を1行はさんでから当該一文を追加する形にした
- 【フォロワー増加のためのUX変更・URLを本体ツイートからリプライへ】「毎日投稿しているのにXのフォロワーが増えない」という相談を受け、外部リンクを含む投稿はXのアルゴリズム上リーチが下がりやすいという通説に基づき、URLの扱いを変更した
  - `body`（投稿文生成結果）に`urlStr`（記事URL付き文字列）を連結する処理を撤廃し、`tweetBox`・「X で投稿」ボタンで開く本体ツイートは常にURLなしで完結するようにした（本文拡張・自動短縮の文字数上限判定も、URL分の23カウントを見込まない本文のみの`POST_CHAR_LIMIT`判定に統一）
  - 結果カードに新しく「🔗 リプライ用」枠（`replyUrlRow`/`replyUrlValue`/`copyUrlBtn`）を追加
  - 「X で投稿」ボタン（`xBtn`）押下時に、URLなし本文でXの投稿画面を開くのと同時にリプライ文を自動でクリップボードにコピーする（Twitter intent URLベースの投稿のため、投稿後のツイートIDをアプリ側で取得できず、自己リプライの自動投稿はできない。運用はユーザーが本体を投稿→自分のツイートにリプライしてクリップボードの内容を貼り付ける、の2ステップ手動）
  - 投稿履歴（`postHistory`/`markPosted`）にはリプライ文を保存していないため、履歴から復元（`loadHistory`）した場合は`replyUrlRow`を非表示にする
  - 検証: `node --check`でJS構文確認、Python構文チェック、サーバー実起動でHTML配信確認。GEMINI_API_KEY未設定のためGemini実呼び出し込みの完全なE2Eは未検証だが、ブラウザでDOM操作により結果カード表示後の状態を再現し、リプライ枠の表示・本体ツイート本文にURLが含まれないこと・「📋 リプライ文をコピー」ボタンの実クリック（実際のユーザー操作扱いでのクリップボード書き込み）が正常動作することを確認済み
- 【リプライ文に「元記事はこちら」を追加】ユーザーから「URLを貼ってリプライするときに、元記事はこちらみたいなのを含めたい」と依頼があり、リプライ枠・自動コピー内容を生URLだけでなく`buildReplyText(url)`が組み立てる「元記事はこちら👇\n{URL}」の2行に変更
  - `.reply-url-value`に`white-space: pre-wrap`を追加し、改行入りのリプライ文が枠内で正しく2行表示されるようにした
  - ボタンラベルも「📋 リプライ用URLをコピー」→「📋 リプライ文をコピー」に変更（コピー内容が生URLではなくラベル付き文になったため）
  - 検証: Node.jsで`buildReplyText()`を単体実行し、URLありの場合に`"元記事はこちら👇\nhttps://..."`、URLなしの場合に空文字を返すことを確認。ブラウザのDOM描画でも2行表示になることを確認済み
- 【フォロワー増加のためのブラッシュアップ3件（A→B→Cの順）】「Xのフォロワー増加のためにシステムをブラッシュアップするなら何が効果あるか」の相談を受けて出した案のうち、優先度順に3件実装した
  - **A. リプライ先候補の提示**: 上記「リプライ先候補（フォロワー増加のための導線）」を参照。`OFFICIAL_X_ACCOUNTS`をJS側に複製し、`renderReplyTargets()`で選択カテゴリの公式Xアカウントリンク＋記事タイトルでのX検索リンクを結果カードに表示。API費用なし
  - **B. 投稿の効果測定ループ**: 上記「投稿の効果測定ループ」を参照。`it_post_metrics_log_v1`に投稿ごとの記録を永続化し、表示回数・いいね数を後から手入力→感想スタイル別/カテゴリ別/長さ別に平均集計する仕組みを追加
  - **C. 短文モードの追加**: 上記「投稿文の長さ（長文／短文モード）」を参照。`POST_LENGTHS`設定（長文350〜500字／短文120〜180字）を追加し、感想スタイルの下に選択ピルを設置。短文モードでは自動膨張（`expandThreshold`）をスキップするよう分岐
  - Bの効果測定にCの「投稿の長さ」を3軸目として組み込み、長文/短文どちらが伸びるか実測できるようにした（`addMetricsEntry`に`postLength`引数を追加）
  - 検証: `node --check`でJS構文確認、Python構文チェック、`el()`参照とHTML idの突き合わせ（不整合ゼロ）を実施。サーバー実起動＋ブラウザのDOM操作/実クリックで、(1)リプライ先候補のリンクが正しいURL（`.href`で復号確認）で表示されること、(2)効果測定ループの記録作成→数値入力（`onchange`実クリック）→3軸集計の平均値が手計算と一致すること、(3)短文/長文の切り替えでpillの選択状態が正しく切り替わり、`expandThreshold`が短文時のみ`null`になり自動膨張がスキップされることをNode.jsのロジック単体テストで確認。GEMINI_API_KEY未設定のためGemini実呼び出しを含む完全なE2E（実際のプロンプト生成結果の長さ差など）は未検証
- 【画像生成プロンプトのテンプレート分岐】「現在の画像生成プロンプトをもっとブラッシュアップした方がいいか」という相談を受け、常に同じ「マスコットロボット＋矢印」構図だと(1)記事内容に関わらず絵が代わり映えせず見飽きられやすい、(2)そのスタイル自体が典型的な"AI製インフォグラフィック"の見た目で、末尾に足した「AIで生成したとわからないようにして」の指示と矛盾する、(3)日本語テキストを画像に多く焼き込むほど文字崩れのリスクが上がる、という3点を指摘。このうち(1)のテンプレート多様化を実装した（(2)(3)は今回対応せず、必要になれば別途）
  - 記事内容から`comparison`（比較・変化型、従来通りのマスコット構図）/`stat`（数値インパクト型、巨大な数字を主役にしたstat-card構図）/`announcement`（発表・アップデート型、マスコット無しの落ち着いたエディトリアル風カード構図）の3テンプレートのいずれかをGeminiに判定させ、選んだテンプレートに応じて`buildComparisonImagePrompt()`/`buildStatImagePrompt()`/`buildAnnouncementImagePrompt()`のいずれかで最終プロンプトを組み立てるよう変更（`buildImagePromptFromParsed()`が分岐）。テンプレート判定も含めてGemini呼び出しは従来通り1回のまま（追加コストなし）
  - 各テンプレート固有のJSON出力ルール（`stat`用の`stat_value`/`stat_label_ja`/`stat_context_en`、`announcement`用の`body_ja`/`visual_en`）を1つのプロンプトにまとめて指示し、Geminiが選ばなかったテンプレート用のフィールドは省略可とした
  - 必要フィールドが欠けている場合（例: `template=stat`なのに`stat_value`が空）は`comparison`にフォールバックする安全策を追加
  - 3テンプレート共通の技術仕様（16:9・1200x675・Xの安全マージン）と末尾の「AIで生成したとわからないようにして」は`IMG_BASE_SPEC`/`IMG_FOOTER`として共通化し、テンプレートごとの重複を避けた
  - 検証: Node.jsで`buildImagePromptFromParsed()`を含む一連の関数を切り出して単体実行し、(1)comparison入力→マスコットロボット表現とタイトルが両方含まれる、(2)stat入力→stat-card表現と数値が含まれ`mascot robot`は含まれない、(3)announcement入力→editorial表現と`body_ja`が含まれ`sparkle`は含まれない、(4)stat指定なのに必須フィールド欠落→comparisonへフォールバックする、の4パターンすべて期待通りであることを確認。`node --check`でJS構文確認、Python構文チェック、サーバー実起動＋ブラウザでボタンのイベントハンドラが正しく設定されていること・コンソールエラーが出ないことを確認済み。GEMINI_API_KEY未設定のためGemini実呼び出し込みの完全なE2E（実際に3種類のtemplateが状況に応じて選ばれるかの確認）は未検証
