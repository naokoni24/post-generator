...
- 【アイコン更新】ホーム画面アイコンを白フチ無しのバージョンに差し替え（`apple-touch-icon.png` を物理ファイルとしてリポジトリに追加し、`build_app_icon()` はまずこのファイルを優先して読み込み、無ければ従来のBase64埋め込みにフォールバック）。キャッシュ更新のため `APP_ICON_VERSION` によるクエリ文字列バージョニング（`/apple-touch-icon.png?v=...`、`/manifest.webmanifest?v=...`）を追加
